...
// are draggable (desaturated) so they can be restored or archived. Has
// list <-> block view toggle. Swipe-left on a card archives it.
...
export default function CompletedAccordion({ tasks, categoriesById, onRestore, onArchive, onEdit }) {
  ...
  function cardProps(t) {
    return {
      ...
      source: 'completed',
      onRestore, onArchive, onEdit,
      onSwipeLeft: () => onArchive?.(t.id),
    };
  }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-t border-gray-200/60 dark:border-gray-700/60 pt-3 pb-2">
        <button
          onClick={() => setOpen(o => !o)}
          className="flex items-center gap-2 text-left text-sm text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
        >
        ...
        {open && count > 0 && <ViewToggle view={view} onChange={setView} />}
      </div>
        ...
        <div className="mt-3">
          {count === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {sorted.map(t => <DraggableTaskCard {...cardProps(t)} compact />)}
            </div>
          ) : (
            (() => {
              const groups = {};
